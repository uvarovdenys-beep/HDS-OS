...
- **Workdir staging** — a workdir outside the container VM's shared roots (e.g. an external `/Volumes` disk) is auto-mirrored into a VM-visible dir, run, and written back — otherwise the mount is silently empty. Keeps the OS portable regardless of install location

- **`sandbox/provision.py`** — isolation auto-installer: `python3 -m sandbox.provision --install` puts lima + colima + docker CLI into `~/.local` (no root, no brew) and boots the VM; `SandboxRunner` emits `isolation_missing` with this command whenever it degrades to subprocess

`docker_backend.py` is the only model-reachable file that may use `subprocess` — enforced by `exec_path_audit.py` (provision.py is allowlisted as operator-invoked tooling).
...
## Editor / external-client integration (agent-as-MCP)

- **`mcp_server.py`** — HDS as an MCP server (stdio JSON-RPC, no SDK dependency).
  Exposes the **agent**, not just the cage: `agent_build`, `agent_build_project`,
  `agent_status`, `agent_models`. Every call runs the full hardened pipeline
  (model resolution → canary → language-aware generation → CAGE → watchdog →
  quarantine/model-escalation). The client states intent; it never writes files.
- **Reference examples** — `reference_files` on `agent_build` / `agent_build_project`: small files (paths or inline `{name, content}`) are shown to the model as an EXAMPLE to match — a design system, an API, a house style. Capped (4 files / 4 KB each / 12 KB total), propagated to every project subtask; a bad reference is skipped, never fatal. Showing an example steers a local model far harder than describing it in prose.
- **`agent/task_bridge.py`** — the single enqueue/poll contract shared by every
  front-end (HTTP webhook + MCP). Prevents the drift that once had the webhook
  writing to `results/` while the agent read `active/`.
- Requires the agent daemon: `HDS_SILENT=1 python3 agent/agent.py --monitor`.
- Verified live: `agent_build` returned a cage-verified, functionally correct
  file; an adversarial `eval()` request came back `failed` and never reached disk.
...
- Automatic (unprompted) Docker install — degradation instead *offers* the install
  (`isolation_missing` event); the operator runs `python3 -m sandbox.provision --install` once

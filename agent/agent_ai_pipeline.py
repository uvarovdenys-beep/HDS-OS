...
def _tel(stage, verdict, task_id, **extra):
    """Emit one structured telemetry event. Never breaks a build."""
    try:
        import telemetry
        telemetry.record(task_id, stage, verdict, **extra)
    except Exception:
        pass
...
class AICodePipelineMixin(PipelineHelpersMixin):
    ...
    def _execute_ai_task(self, task_id: str, instruction: str, model_name: str = "",
                         ...
                         reference_files=None, patch_target: str = "",
                         acceptance_tests=None) -> bool:
        """
        ...
        # ORCHESTRATOR SKILLS: the nuance a signature cannot carry — that the
        # target is a method inside a class, that TS needs explicit types, that
        # JS assertions need ===. Measured: without the method rule a 14B
        # returned the whole class four times and the cage refused every one.
        try:
            import skills as _skills
            skills_ctx = _skills.block(
                lang, patch_target, (self.BASE_DIR / rel_path).exists()
                if 'rel_path' in dir() else True)
        except Exception:
            skills_ctx = ""
        # WORKED BEFORE: verified functions similar to this task. Only fully
        # verified code is in the library, so an example here is proof, not a
        # guess — the positive half of memory, beside the anti-patterns.
        try:
            import skill_library as _lib
            worked_ctx = _lib.block(instruction, lang)
        except Exception:
            worked_ctx = ""
        base_prompt = (
            ...
            f"{skills_ctx}{worked_ctx}{refs_ctx}{siblings_ctx}\n"
            f"Task: {instruction}"
        )
        ...
        import scribe
        tries = self._self_correct_tries()
        ...
        # RECALL (semantic episodic memory): remind the model of relevant past
        # mistakes before it writes, favouring lessons anchored to this file.
        # Empty unless a lesson clears the similarity floor — no irrelevant
        # advice leaks. Best-effort; never break a build over recall.
        try:
            _sym = f"{rel_path}::{patch_target}" if patch_target else ""
            _recall = self.experience.get_context_for_prompt([instruction], symbol=_sym)
            if _recall:
                base_prompt = _recall + base_prompt
        except Exception:
            pass

        # SNAPSHOT before the first write. scribe must write before acceptance
        # and Monte Carlo can run, so a task that ends up failing has already
        # changed the file. Without this, a rejected generation stays on disk
        # looking like finished work.
        try:
            import rollback as _rb
            _before = _rb.snapshot(self.BASE_DIR / rel_path)
        except Exception:
            _rb, _before = None, None

        escalated = False
        for attempt in range(1, tries + 1):
            # ESCALATE: retrying a model that has already failed twice on the
            # same feedback is waste. Move up the ladder to a stronger served
            # model once, then keep self-correcting with it.
            if attempt > 2 and not escalated and feedback:
                try:
                    stronger = self._escalate_to(model_name, self._served_model_ids())
                except Exception:
                    stronger = ""
                if stronger and stronger != model_name:
                    resolved2 = self._resolve_model(stronger)
                    if resolved2:
                        provider, model_name, ai_call = resolved2
                        self._enforce_single_model(
                            model_name if provider == "ollama" else "")
                        gate = (self._trusted_models.get(model_name, "L") or "L").lower()
                        escalated = True
                        _tel("escalate", "start", task_id,
                             attempt=attempt, model=model_name)
                        self.vox.speak(
                            f"Escalating to {model_name} after {attempt - 1} "
                            f"failed attempts", "INFO")
            prompt = base_prompt
            if feedback:
                # In function mode the model must re-emit ONE function, not the
                # file — telling it otherwise is how a surgical patch turns
                # back into a whole-file rewrite.
                unit = (f"corrected function '{patch_target}'" if patch_target
                        else "FULL corrected file")
                prompt += (f"\n\nYOUR PREVIOUS ATTEMPT WAS REJECTED. Output the "
                           f"{unit} that fixes EXACTLY this:\n{feedback}\n")
            _tel("generate", "start", task_id, attempt=attempt, lang=lang)
            ...
            try:
                from task_tree import extract_target, write_op
                # The model is TOLD to emit one function and does not obey — a
                # 14B asked for is_open returned the whole module, which then
                # appended as duplicate definitions and made the next patch
                # ambiguous. Cut the requested function out instead of trusting
                # the instruction: constrain, do not instruct.
                if patch_target and is_python:
                    code = extract_target(code, patch_target)
                _target_file = self.BASE_DIR / rel_path
                _existing = (_target_file.read_text(encoding="utf-8")
                             if _target_file.exists() else "")
                scribe.execute(
                    write_op(rel_path, code, patch_target,
                             _target_file.exists(), _existing),
                    protocol_size=gate if gate in ("s", "m", "l", "xl") else "l")
            except scribe.ScribeError as e:
                feedback = str(e)
                _tel("cage", "fail", task_id, attempt=attempt, reason=str(e)[:120])
                # Append a plain instruction on HOW to satisfy the rule — cage
                # verdicts are terse, and cage-unsatisfiable is the top give-up
                # cause (hds_failures). Best-effort; never break on the hint.
                try:
                    from cage_help import explain as _cage_explain
                    _hint = _cage_explain(str(e))
                    if _hint:
                        feedback += "\n" + _hint
                except Exception:
                    pass
                ...
                continue
            # 5b. ACCEPTANCE. Checks declared in the PLAN, executed against the
            # code. This is the one Monte Carlo cannot do: `return a - b` never
            # crashes, so randomised calls pass it. Measured — 100 random calls,
            # verdict OK, the function wrong. Declared checks caught it.
            # Acceptance runs for any language acceptance.check_module can
            # verify (Python, JavaScript). Others return 'unverified' and
            # never fail the build.
            if acceptance_tests:
                import acceptance as _acc
                verdict = _acc.check_module(rel_path, acceptance_tests)
                self.vox.speak(_acc.summarise(verdict), "INFO")
                if not verdict.get("ok"):
                    feedback = _acc.feedback(verdict)
                    _tel("acceptance", "fail", task_id, attempt=attempt)
                    self.vox.speak(f"Acceptance failed (attempt {attempt})", "ERROR")
                    continue

            # 5c. MONTE CARLO. The cage proves the code is safe and well-formed;
            ...
            # crashes and hangs, not wrong-but-stable logic. Runs for any
            # language montecarlo supports (Python, JavaScript, TypeScript);
            # others return 'unverified' and never fail the build.
            if True:
                verdict = self._monte_carlo(rel_path)
                if verdict is not None and not verdict.get("ok"):
                    _tel("montecarlo", "fail", task_id, attempt=attempt)
                    ...
                    continue
            _tel("cage", "ok", task_id, attempt=attempt)
            _tel("written", "ok", task_id, attempt=attempt, path=rel_path)
            # Admit to the skill library ONLY here: the cage passed, the declared
            # assertions held and Monte Carlo ran it. An unproven example would
            # propagate its own mistake to every later task.
            if patch_target and acceptance_tests:
                try:
                    import skill_library as _lib
                    _lib.record(patch_target, code, lang,
                                contract=instruction[:200],
                                symbol=f"{rel_path}::{patch_target}")
                except Exception:
                    pass
            ...
            _tel("gave_up", "fail", task_id, attempt=tries)
            # Nothing earned its place on disk — put the file back.
            if _rb is not None:
                if _rb.restore(self.BASE_DIR / rel_path, _before):
                    self.vox.speak(f"Rolled back: {_rb.describe(_before)}", "INFO")
                    _tel("rollback", "ok", task_id, attempt=tries)
                else:
                    self.vox.speak("Rollback FAILED — the rejected file is still "
                                   "on disk", "ERROR")
                    _tel("rollback", "fail", task_id, attempt=tries)
            return False

        output_file = self.BASE_DIR / rel_path
        self.vox.speak(f"Code written: {output_file.name}", "INFO")

        # RECORD (Reflexion, episodic memory): a generation that was rejected and
        # then fixed is a lesson worth keeping. `feedback` holds the last
        # rejection the winning attempt corrected; attempt>1 means a correction
        # happened. Best-effort — memory must never break a completed build.
        try:
            import reflexion
            _lesson = reflexion.lesson_from_error(task_id, feedback, attempt)
            if _lesson:
                _sym = f"{rel_path}::{patch_target}" if patch_target else ""
                self.experience.register_failure(**_lesson, symbol=_sym)
        except Exception:
            pass
        ...
        if decompose_result:
            self.vox.speak(f"Auto-decomposed into {len(decompose_result.extracted)} files", "INFO")

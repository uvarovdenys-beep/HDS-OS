...
def check_and_decompose(filepath: str, model_size: str = "m") -> Optional[DecomposeReport]:
    """Integration point for ProtocolEnforcer: decompose `filepath` if it
    exceeds the line limit for `model_size` ("xl"/"l"/"m"/"s").

    PYTHON ONLY — decomposition runs ast.parse, so a .ts/.css file raised
    SyntaxError and killed the agent daemon mid-task. Other languages have no
    decomposer yet, so they are skipped rather than parsed as Python.

    Returns a DecomposeReport if decomposition happened, else None.
    """

...
class SymbolInfo:
    ...
    calls: List[str] = field(default_factory=list)     # Names this symbol invokes (code-graph edge)
...
def scan_module(filepath: str) -> ModuleInfo:
    ...
    Returns compact info: module doc, classes, functions, their docs, and the
    symbols each one calls (the code-graph edges).
    """
...
def build_index(agent_dir: str = ".") -> Dict:
    ...
    for path in sorted(Path(agent_dir).iterdir()):
        fname = path.name

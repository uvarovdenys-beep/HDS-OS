# vox.py
# HDS Voice/Text Notification Service with PowerVox Speech
...
try:
    from vox_speech import HDSVoxSpeech
except ImportError:
    HDSVoxSpeech = None
...
class VoxConfig:
    ...
    def load(cls, base_dir: Path) -> Dict:
        ...
        config_path = base_dir / "ai-mind" / "knowledge" / "hds-config.json"
...
class VoxService:
    ...
    def __init__(self, log_dir: Path = None, enable_speech: bool = None):
        ...
        if enable_speech is None:
            # Use configuration
            self.speech_enabled = config_speech_enabled and HDSVoxSpeech is not None
        else:
            # Forced parameter (override)
            self.speech_enabled = enable_speech and HDSVoxSpeech is not None
        
        # Voice selection from config
        config_voice = self.config.get("speech", {}).get("voice", "sonia")
        self.speech = HDSVoxSpeech(self.base_dir, voice=config_voice) if self.speech_enabled else None

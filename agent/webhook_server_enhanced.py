...
# Build tasks a client submits must run through the HARDENED agent pipeline
# (cage, watchdog, quarantine, coherence). The enqueue/poll contract lives in
# agent/task_bridge.py so every front-end (HTTP, MCP) shares ONE implementation.
try:
...
async def submit_task(task_data: dict):
    ...
    # Build tasks → the shared bridge enqueues them for the agent, which drives
    # them through the full hardened pipeline (cage/watchdog/quarantine).
    # One bridge, used by every front-end (HTTP here, MCP elsewhere).
    if task_bridge.is_build_task(task_data):
        task_data.setdefault("task_id", task_id)
        return task_bridge.submit(task_data)
...
async def get_task_status(task_id: str):
    """Get task status via the shared bridge (same contract the MCP front-end
    sees): completed/failed/running, else the inline results record."""
    s = task_bridge.status(task_id)
    if s is None:

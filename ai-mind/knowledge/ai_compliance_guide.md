# AI Compliance Guide
**Як перевіряти дії ШІ на відповідність HDS**
...
```python
from agent.compliance import HDSComplianceChecker, ViolationLevel
...
if result.level == ViolationLevel.VIOLATION:
    print("DIRECT WRITE DETECTED - use HDSScribe instead")
```
...
**Виправлення:**
```python
from agent.scribe import HDSScribe
scribe = HDSScribe()
scribe.write_file(
...
class AIActivityMonitor:
    ...
    def __init__(self):
        self.checker = HDSComplianceChecker()
        self.violations_log = []
...
class AIController:
    """Контролер дій ШІ з HDS compliance."""
...
**Integration**: `from agent.compliance import HDSComplianceChecker`
**Test**: `python agent/compliance.py`
**Version**: HDS AI Compliance v1.0

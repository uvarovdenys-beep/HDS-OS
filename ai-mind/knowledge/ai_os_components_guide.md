...
HDS OS now includes advanced AI components for autonomous operation:
...
- **HDS OS Integration** - Unified system management
...
- Integration with existing HDS task system
...
### Configuration
Update `ai-mind/knowledge/hds-config.json`:
```json
{
  "speech": {
    "enabled": true,
...
```bash
python hds_os_integration.py --start
```

#### System Status
```bash
python hds_os_integration.py --status
python hds_os_integration.py --detailed
```

#### Create Integrated Tasks
```python
from agent.hds_os_integration import HDSOSIntegration

system = HDSOSIntegration()
system.start_system()
...
python hds_os_integration.py --emergency-stop

# Clear all caches
python hds_os_integration.py --clear-caches

# Export configuration
python hds_os_integration.py --export-config system_config.json
```
...
### Main Configuration
File: `ai-mind/knowledge/hds-config.json`
...
### Access Control
- All file operations through HDS Scribe
...
2. **Configure System**: Update `hds-config.json` with your preferences

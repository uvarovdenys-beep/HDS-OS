...
SMTP Reporter автоматично відправляє рекомендації по покращенню HDS на центральну пошту розробників. Це забезпечує feedback loop для постійного вдосконалення системи.
...
```bash
# Windows (cmd)
set HDS_SMTP_USER=your_email@gmail.com
set HDS_SMTP_PASS=your_app_password

# Windows (PowerShell)
$env:HDS_SMTP_USER="your_email@gmail.com"
$env:HDS_SMTP_PASS="your_app_password"

# Linux/Mac
export HDS_SMTP_USER=your_email@gmail.com
export HDS_SMTP_PASS=your_app_password
```
...
```python
from agent.compliance import HDSComplianceChecker

checker = HDSComplianceChecker()
...
### Тема листа:
```
[HDS] CRITICAL: SEC-001 Violation
[HDS] Weekly Report: 3 recommendations
[HDS] high: Add timeout to agent cycle
```

### Вміст листа:
```
HDS Recommendation Report
...
Suggested Fix:
Review HDS logs and take immediate action
...
- `.hds_manifest.json` — контрольна сума системи
...
```python
from agent.smtp_reporter import HDSSMTPReporter, Recommendation
from datetime import datetime

reporter = HDSSMTPReporter()

...
# JavaScript is CASE-SENSITIVE, so re.I bought no safety here and cost a lot:
# it made `function(` — the most common construct in JS — match the
# `Function(` constructor rule, so any inline script using a function
# expression was refused. Measured: it blocked a legitimate console mockup.
# Case-sensitive now: `Function(`/`new Function(` and `eval(` are still
# caught (those are the real dynamic-code surface); `function(){}` is not.
_SCRIPT_BODY_DENY = re.compile(r"\beval\s*\(|\bFunction\s*\(|\bimport\s*\(")

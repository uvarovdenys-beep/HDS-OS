...
INSTALL = {
    ...
    "bash":    "bash ships with macOS and every linux — check PATH",
    "ruby":    "ruby ships with macOS; apt install ruby  (linux)",
    "gofmt":   "install Go — https://go.dev/dl (gofmt ships with it)",
    "rustc":   "curl https://sh.rustup.rs -sSf | sh   (installs into ~/.cargo)",
    "javac":   "install a JDK — https://adoptium.net (macOS /usr/bin/javac is a "
               "stub that only points at java.com)",
}
_offered = set()

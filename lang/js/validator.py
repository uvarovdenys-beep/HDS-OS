...
@register(".js", ".mjs", ".cjs", ".ts", ".jsx", ".tsx", kind="exec")
def validate_js(content, path):
    ...
    if ext in (".ts", ".tsx"):
        ...
        return
    # .js / .mjs / .cjs / .jsx — hygiene picked by RUNTIME (the real
    # browser-vs-node concern); syntax is runtime-agnostic.
    # .mjs is ESM and .cjs is CommonJS: both are Node by definition, so they
    # never take the browser denylist.
    node_js = ext in (".mjs", ".cjs") or _is_node_js(content)
    deny_scan(content, path, _NODE if node_js else _BROWSER)
    # node --check used purely as an ES SYNTAX parser (NOT a runtime assertion);
    # .jsx is not plain ES, so it is not parsed here.
    if ext in (".js", ".mjs", ".cjs"):
        check(["node", "--check"], content, path, suffix=ext,

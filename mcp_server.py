...
# The cage as a write surface, and the three-level plan as external memory.
# An editor already has a good model; what it does not have is a writer that
# refuses to clobber the rest of a file, or somewhere to keep a plan that its
# context window is not paying for on every turn.
import cage_tools  # noqa: E402
...
def _advertised():
    ...
    Every schema is re-sent in the system prompt on every turn, so tools that
    will not be used are a standing tax on the context window: the five agent_*
    tools cost ~740 tokens and need a running daemon. Set HDS_MCP_MINIMAL=1 for
    the editor path — the cage and the plan, nothing else.
    """
    import os as _os
    if _os.environ.get("HDS_MCP_MINIMAL", "0") == "1":
        return cage_tools.TOOLS + tree_tools.TOOLS
    return TOOLS + cage_tools.TOOLS + tree_tools.TOOLS
...
def _build(args: dict, project: bool) -> dict:
    if not _daemon_awake():
        return {"status": "no_agent",
                "detail": "The HDS agent daemon is not running, so this build "
                          "would sit in the queue until it timed out.",
                "start_it": "cd HDS_CORE && HDS_SILENT=1 python3 agent/agent.py --monitor &",
                "note": "cage_write / cage_patch / cage_insert and every tree_* "
                        "tool work WITHOUT the daemon — only agent_build needs it, "
                        "because it asks a local model to generate the code."}
...
def _daemon_awake() -> bool:
    """Is the agent daemon draining the queue?

    Heuristic, and deliberately so: the exec surface is sealed to the sandbox,
    so this cannot look for a process. The daemon logs every cycle (~60s), so a
    log older than three minutes means nobody is listening. False positives cost
    a wrong hint; the alternative — enqueueing into a dead queue — costs the
    caller a ten-minute timeout.
    """
    from pathlib import Path as _P
    import time as _t
    log = _P(__file__).resolve().parent / "ai-mind" / "logs" / "vox.log"
    try:
        return (_t.time() - log.stat().st_mtime) < 180
    except OSError:
        return False


def call_tool(name: str, args: dict) -> dict:
    # Cage and plan ops first: they are why an editor connects to HDS at all.
    if name in cage_tools.TOOL_NAMES:
        return cage_tools.call(name, args)
    if name in tree_tools.TOOL_NAMES:
        return tree_tools.call(name, args)
    if name == "agent_build":

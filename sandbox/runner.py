...
class SandboxRunner:
    ...
    def _auto_backend() -> "SandboxBackend":
        ...
        if docker.available():
            return docker
        # Degradation must be visible + self-remediating (like toolchain_missing):
        # offer() emits `isolation_missing` with the exact install command.
        from .provision import offer
        offer()

...
# HDS Agent Startup Script - WITHOUT AUDIO
# Запускає основне ядро з усіма 6 модулями БЕЗ ЗВУКУ
...
echo "═══════════════════════════════════════════════════════════"
echo "  HDS NUCLEUS - SILENT MODE (NO AUDIO)"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "Components:"
...
echo ""
echo "Starting HDS Agent (SILENT MODE)..."
echo "═══════════════════════════════════════════════════════════"
echo ""

cd "$BASE_DIR"

# Set silent mode environment variable
export HDS_SILENT=1

# Запускаємо агент
if [ "$1" == "--monitor" ]; then
    echo "Mode: MONITOR (continuous) - SILENT"
    python3 -c "
import sys
import os
...
agent.monitor()
    ...
    echo "Mode: ONCE (single cycle) - SILENT"
    python3 -c "
import sys
import os
os.environ['HDS_SILENT'] = '1'
...
agent.run()
    ...
    echo "Mode: INTERACTIVE - SILENT"
    echo ""
    echo "Usage:"
    echo "  ./start_hds_agent_silent.sh --monitor   (continuous mode)"
    echo "  ./start_hds_agent_silent.sh --once      (single cycle)"
    echo ""
    python3 -c "
import sys
import os

...
def test_unvalidated_language_default_denied():
    """A gated extension with no validator must be denied.

    Uses a FAKE extension: this test named '.go', and when Go gained a validator
    it began failing even though the rule it guards never changed. Pin the rule,
    not a language's current support status.
    """
    original = scribe.CODE_EXTS
    scribe.CODE_EXTS = original + (".zzz",)
    try:
        scribe.execute({"op": "write", "path": "storage/a.zzz", "content": "x"}, "l")
        assert False, "unvalidated extension was not default-denied"
    except scribe.ScribeError:
        pass
    finally:
